...
template_file   = 'submissions/sample_submission.csv'
...
def main(options):
  ...
  h_games_played = seed_prob_file.Get('h_games_played')
  h_seed_winprob = seed_prob_file.Get('h_seed_winprob')
      
  outcomes    = []
  predictions = []  
  
  fout = open('submissions/seedprob_submission.csv','w')
  ...
  with open(template_file,'r') as f:     
    ...
    for line in f:
      ...
      if h_games_played.GetBinContent(seed_team1, seed_team2) >= 10:
        prediction = h_seed_winprob.GetBinContent(seed_team1, seed_team2)
